...
public class A02_String {
	...
	public static void main(String[] args) {
		...
		2. 문자열의 비교
			1) 주소값 비교 원칙(자바원칙)
				...
				== : stack영역의 데이터를 비교하여 boolean값을
					...
					에 생성이 된다.
				처음에는 "홍길동"을 만들고 특정한 heap영역의 위치에
				할당, 그 다음 "홍길동" 처음에 만들었던 주소값을 그대로
				사용해서 변수에 할당한다.
				주의) 자바는 대부분 선언을 해서 문자여을 사용하는 것이
				아니라, 입력받던지, 파일에 있는 데이터를 로딩하든지,
				네트웍상으로 데이터를 가져오든지 해서 실제로는 new String()
		...
		3. String 기능 메서드
			1) 생성자
				new String()
				new String(byte[]) {24,75} ==> {'홍','길'} ==> 홍길
					최종프로젝트에 활용 아이디를 잊었을때, 임시 비번생성
		...
		// "홍길동" : 문자열 바로 대입한것은 문자열이 같은 경우 모두 주소가 같음
		System.out.println("str1==str2 : "+ (str1==str2)); //==> true(주소값비교)
		...
		// ex) "Lunch"의 각 byte코드값을 확인하여 위와같이 배열을 만들어
		//		문자열을 출력하ㅔ요
		
		byte[] bytes01 = {34,76,117,110,99,104,34};
		String lunch = new String(bytes01);
